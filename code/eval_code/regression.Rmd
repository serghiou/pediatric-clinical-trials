...
regression %>%
  ...
  mutate(n_trials_diff = `More developed` - `Less developed`) %>% 
...
regression %>%
  ...
  ylab("Number of trials in more - less developed")
```
...
regression %>% 
  filter(dev != "all") %$% 
  cor.test(n_trials, dalys, method = "spearman") %>% 
  broom::tidy() %>% 
  sable()
```
