...
```{r}
regression %>%
  mutate(sample_size = sample_size + 1) %>% 
  ggplot(aes(x = dalys, y = sample_size)) +
  # geom_text_repel(
  #   aes(label = level_three), 
  #   size = 3, 
...
regression %>%
  filter(dev != "All") %>% 
  ggplot(aes(x = dalys, y = sample_size, color = dev, fill = dev)) +
  geom_line() +
  geom_point() +
  labs(x = "Log-transformed DALYs (millions)", y = "Total sample size") +
  scale_x_continuous(
...
regression %>%
  filter(dev != "All") %>% 
  pivot_wider(id_col = dalys, names_from = dev, values_from = sample_size) %>% 
  mutate(sample_size_diff = `More developed` - `Less developed`) %>% 
  ggplot(aes(x = dalys, y = sample_size_diff)) +
...
regression %>%
  ...
  ylab("Total sample size in more - less developed")
```
...
regression %>%
  ...
  ggplot(aes(x = dalys, y = sample_size_diff, group = daly_groups)) + 
...
# Save
ggsave(
  "../../output/figure_output/regression_binned.jpg",
  width = 13,
  height = 8
)
```
...
# Save
ggsave(
  "../../output/figure_output/regression_log_poisson_completed.jpg",
  width = 13,
  height = 8
)
```
...
# Save
ggsave(
  "../../output/figure_output/regression_log_poisson_enrollment_completed.jpg",
  width = 13,
  height = 8
)
```
...
glm.linear <- 
  ...
  lm(sample_size ~ dalys + dev + dalys * dev, .)
...
glm.lognormal <- 
  ...
  filter(dev != "All") %>% 
  mutate(dalys = dalys / 10^6) %>% 
  glm(log(sample_size + 1) ~ dalys + dev + dalys * dev, ., family = gaussian())

glm.gamma <- 
  regression %>% 
  filter(dev != "All") %>% 
  mutate(dalys = dalys / 10^6) %>% 
  glm(sample_size + 1 ~ dalys + dev + dalys * dev, ., family = Gamma(link = "log"))
```
...
This is a descriptive model, not a model appropriate for prediction or inference. Overall, with every million DALYs, there was a mean increase in total sample size ranging from 261 to 694 (Sandwich p-value, 0.00006). Even though, overall, the mean total sample size in more developed countries was smaller than that of less developed countries, these data do not suggest a simple linear relationship (Sandwich p-value, 0.6).

<!-- Interpretation of the Poisson model: This table suggests that for every a million DALYs, the expected sample size increases by a median of 1.01 (95% CI, 1.008-1.02; P-value = 0.00002). It also suggests that in the observed trials, the sample size of studies from more developed countries was a median of 51% smaller than that of less developed countries, across levels of DALYs. However, this analysis does not suggest that this is generally true (95% CI, 0.17-1.44). No meaningful effect modification between DALYs and development was observed (95% CI, 0.99-1.01). -->
