...
regression %>%
  ...
  geom_smooth(method = MASS::glm.nb, aes(color = "NB"), se = F) +
  geom_smooth(
    method = "lm", 
    aes(color = "LM"), 
    formula = y ~ log(x), 
  ...
  geom_smooth(method = "lm", formula = y ~ x, aes(color = "Original"), se = F)
  geom_text_repel(aes(label = level_three), size = 2, segment.size = 0.2) +
  facet_wrap(~ dev, nrow = 2) +
  labs(x = "DALYs (millions)", y = "Total sample size") +
  scale_x_continuous(labels = scales::unit_format(unit = "M", scale = 1e-6)) +
...
# Save
ggsave(
  "../../output/figure_output/regression_original_with-enrollment.jpg",
  width = 13,
  height = 8
)
```
...
# Save
ggsave(
  "../../output/figure_output/regression_log_with-enrollment.jpg",
  width = 13,
  height = 8
)
```
