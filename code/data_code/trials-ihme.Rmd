...
# Import data
trials <- read_excel(
  "../../data/interim_data/categorizations_08132020_conditions.xlsx",
  sheet = 1
)

trials_by_income <- read_excel(
  "../../data/interim_data/trials-by-income-status.xlsx",
  sheet = 1
)
```
...
trials %<>%
  drop_na(nct_id) %>% 
  rename(
    brief_summary = `brief_summary...6`,
    ihmelevel3C_code = `ihmelevel3C _code`
  ) %>% 
  select(-`Email_Y (Reply Y/N)`)
```
...
```{r}
trials %<>%
  left_join(trials_by_income) %>% 
  select(-development_status, -development_status_investigators) %>% 
  rename(country_income_class = development_status_participants_WORLDBANK) %>% 
  mutate(country_income_class = case_when(
    ...
    country_income_class == "notreported" ~ "Unreported",
