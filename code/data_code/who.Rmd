---
title: 'Pediatric clinical trials: Prepare WHO GBD data for analysis'
...
# Import data
trials <- read_excel(
  "../../data/interim_data/SC_MasterCategorizations_07012020_conditions.xlsx",
  sheet = 1
)
...
who %<>% 
  rename(
    total_male_dalys = all_age_male,
    total_female_dalys = all_age_female,
    total_both_dalys = all_age_both
  )
```
...
Note that the the dashes are of two different types, this is why both were included below.

```{r}
who %<>% 
  mutate_all(str_replace_all, "·", ".") %>% 
  mutate_all(str_replace_all, "([0-9]) ([0-9])", "\\1\\2") %>% 
  mutate_all(na_if, "..") %>% 
  mutate_all(na_if, "-") %>% 
  mutate_all(na_if, "−")
```
...
```{r}
who %<>% mutate_at(vars(ID), as.character)
```
...
# Create new columns
who %<>% 
  separate(total_male_dalys, into = col_names, sep = " \\(| to ") %>% 
  mutate_at(vars(total_male_dalys_hi), str_remove, "\\)") %>% 
  mutate_at(vars(starts_with("total_male")), as.numeric)
```
...
col_names <- c(
  "total_female_dalys", 
  "total_female_dalys_lo", 
  "total_female_dalys_hi"
)

# Create new columns
who %<>% 
  separate(total_female_dalys, into = col_names, sep = " \\(| to ") %>% 
  mutate_at(vars(total_female_dalys_hi), str_remove, "\\)") %>% 
  mutate_at(vars(starts_with("total_fem")), as.numeric)
```


DALYs for both males and females.
...
# Create new columns
who %<>% 
  separate(total_both_dalys, into = col_names, sep = " \\(| to ") %>% 
  mutate_at(vars(total_both_dalys_hi), str_remove, "\\)") %>% 
  mutate_at(vars(starts_with("total_both")), as.numeric)
```


Relative DALYs to total.

```{r}
# Total DALYs
total_dalys <- 
  who %>% 
  filter(is.na(level_two)) %>% 
  pull(total_both_dalys)

# Relative DALYs
who %<>% 
  mutate(relative_both_dalys = total_both_dalys / total_dalys) %>% 
  relocate(relative_both_dalys, .after = total_both_dalys_hi)
```
...
```{r save}
write_csv(who, "../../data/tidy_data/who.csv")
```
