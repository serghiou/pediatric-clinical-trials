---
title: 'Pediatric clinical trials: Prepare IHME data for analysis'
...
# Import data
level_three <- read_excel(
  "../../data/external_data/ihme-cookbook.xlsx",
  sheet = "Level_three"
)

level_four <- read_excel(
  "../../data/external_data/ihme-cookbook.xlsx",
  sheet = "Level_four"
)
```
...
ihme <- 
  level_three %>% 
  bind_rows(level_four, .id = "level") %>% 
  mutate(level = if_else(level == "1", "level_three", "level_four")) %>% 
  select(ihme_id, who_id, level, condition, dalys)
```
...
```{r save}
write_csv(ihme, "../../data/tidy_data/ihme.csv")
```
